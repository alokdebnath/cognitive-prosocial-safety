...
def get_data(train_path, dev_path, test_path, random_seed):
    ...
    val_df = train_df[train_df.split == "val"].drop_duplicates(subset=['prompt','response']).reset_index()
    train_df = train_df[train_df.split == "train"].drop_duplicates(subset=['prompt','response']).reset_index()
    test_df = pd.read_csv(test_path)
    
    train_df['text'] = '#Context: ' + train_df['prompt'] + '\n\n' + '#Response: ' + train_df['response']
    train_df['label'] = [1 if x == "Safe" else 0 for x in train_df['label']]
    
    val_df['text'] = '#Context: ' + val_df['prompt'] + '\n\n' + '#Response: ' + val_df['response']
    val_df['label'] = [1 if x == "Safe" else 0 for x in val_df['label']]
    
    test_df['text'] = '#Context: ' + test_df['prompt'] + '\n\n' + '#Response: ' + test_df['response']
    test_df['label'] = [1 if x == "Safe" else 0 for x in test_df['label']]
...
if __name__ == '__main__':
    ...
    if True:    
      ...
      if args.test_only != True:
        fine_tune(train_df[['text', 'label']], valid_df[['text', 'label']], f"models/diasafety_{model}", id2label, label2id, model, args.continue_train)
      ...
      results, predictions, probs, class_prob = test(test_df[['text', 'label']], f"models/diasafety_{model}/best/", id2label, label2id)
