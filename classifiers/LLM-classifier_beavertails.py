...
def get_data(train_path, dev_path, test_path, random_seed):
    ...
    train_df['text'] = '#Context: ' + train_df['prompt'] + '\n\n' + '#Response: ' + train_df['response']
    train_df['label'] = train_df.is_safe.astype(int)
    
    #val_df['text'] = '#Context: ' + val_df['prompt'] + '\n\n' + '#Response: ' + val_df['response']
    #val_df['label'] = val_df.is_safe.astype(int)
    val_df = train_df.groupby(['label']).apply(lambda x: x.sample(min(1000, len(x)), random_state = RANDOM_SEED)).sample(frac=1., random_state = RANDOM_SEED).reset_index(drop=True)
    train_df = train_df[~train_df.text.isin(val_df.text)].sample(frac=1., random_state = RANDOM_SEED).reset_index(drop=True)
    
    test_df['text'] = '#Context: ' + test_df['prompt'] + '\n\n' + '#Response: ' + test_df['response']
    test_df['label'] = test_df.is_safe.astype(int)
...
if __name__ == '__main__':
    ...
    if True:    
      ...
      if args.test_only != True:
        fine_tune(train_df[['text', 'label']], valid_df[['text', 'label']], f"models/beavertails_{model}", id2label, label2id, model, args.continue_train)
      ...
      results, predictions, probs, class_prob = test(test_df[['text', 'label']], f"models/beavertails_{model}/best/", id2label, label2id)
